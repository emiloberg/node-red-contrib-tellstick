	...
	if (invocationQueue.length === 1) {
		invocationInterval = setInterval(function () {
			if (invocationQueue.length > 0) {
				var curMsg = invocationQueue[0].msg;
				var curCb = invocationQueue[0].cb;
				invocationQueue.shift();
				invokeTellstickMethod(curMsg, curCb);
			} else {
				clearInterval(invocationInterval);
			}
		}, Settings.outputThrottle);
	}
}
...
module.exports = function(RED) {
	...
	function TellstickOutNode(n) {
		...
		this.on('input', function(incomingMsg) {
			...
			queueTellstickMethod(msg, function(err) {
				if (err) {
	...
	RED.httpAdmin.get('/tellstick/invoke/:deviceid/:method/:dimlevel?', function(req, res) {
		...
		queueTellstickMethod(out, function(err, status) {
			if (err) {
