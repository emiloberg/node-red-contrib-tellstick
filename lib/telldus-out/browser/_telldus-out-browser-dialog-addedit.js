...
function saveAddEdit() {
	 ...
	 */
	var errStr = [];
	var jqSelectedBrands = $('#telldus-select-supported-brands :selected');
	var jqSelectedDevice = $('#telldus-select-supported-devices :selected');

	$('#telldus-dialog-add-edit-device .input-error').removeClass('input-error');
	...
	if (jqSelectedDevice.val().length > 0 && jqSelectedBrands.val().length > 0) {
		...
		if (jqInputCode.data('use') === 'true') {
			...
			$('#telldus-row-addedit-parameter-code input:checked').each(function () {
				if ($(this).val() === 'true') {
	...
	jqXHR.fail(function(jqXHR) {
		RED.notify('<strong>Telldus error (err: 4)</strong>: ' + jqXHR.responseText, 'error');
	});
