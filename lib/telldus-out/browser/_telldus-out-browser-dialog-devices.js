...
function getAllDevicesAndPopulateLists() {
	$.getJSON('telldus/device', function(data) {
		devices = data;
...
jqBody.on('click', '.telldus-invoke-method-link', function() {
	...
	$.getJSON('telldus/invoke/' + outObj.id + '/' + outObj.method + '/' + outObj.value)
		.done(function(data) {
		...
		.fail(function(data) {
			RED.notify('<strong>Telldus error (err: 7): </strong>' + JSON.parse(data.responseText).errWarnStr, 'error');
		});
});

jqBody.on('click', '.telldus-config-device-button', function() {
	$.getJSON('telldus/device/' + $(this).data('deviceid'))
		.done(function(data) {
			console.log('YAY');
			console.dir(data);
		})
		.fail(function(data) {
			RED.notify('<strong>Telldus error (err: 7): </strong>' + JSON.parse(data.responseText).errStr, 'error');
		});
