...
// Todo, check all jQuery selectors, they will catch _ALL_ nodes, not just this one.
RED.nodes.registerType('telldus-input', {
	category: 'config',
	defaults: {
	...
	oneditprepare: function() {
		 ...
		 */
		window.telldus = window.telldus || {};
		window.telldus.inputConfigListener = function(topic, rawdata) {
			...
			var html = '<tr class="method-' + data.method + '">' +
				'<td class="first">' +
				'<button data-data="' + rawdata + '" type="button" id="" class="learn-button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" role="button" aria-disabled="false">' +
		 ...
		 */
		RED.comms.subscribe('telldus-transmission', window.telldus.inputConfigListener);

		// todo change to get status
//            $.getJSON('telldus/send-raw-data-over-ws/start', function(data) {
//                if (data.status > 0) {
...
//            });
	...
	oneditcancel: function() {
		RED.comms.unsubscribe('telldus-transmission', window.telldus.inputConfigListener);
	},
	oneditsave: function() {
		RED.comms.unsubscribe('telldus-transmission', window.telldus.inputConfigListener);
	},
	ondelete: function() { // todo: remove this. ondelete will be deprecated in favor of oneditdelete. https://github.com/node-red/node-red/issues/561#issuecomment-74238250
		RED.comms.unsubscribe('telldus-transmission', window.telldus.inputConfigListener);
	},
